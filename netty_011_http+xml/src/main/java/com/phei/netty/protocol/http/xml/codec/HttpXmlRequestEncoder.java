...
public class HttpXmlRequestEncoder extends
	...
	ByteBuf body = encode0(ctx, msg.getBody());//将pojo编码成xml
	FullHttpRequest request = msg.getRequest();
	//如果request请求为null，就自己封装一个request对象，将xml数据写到这个request中，很显然从HttpXmlClientHandle的ctx.writeAndFlush(request)里的request是NULL值
	if (request == null) {
